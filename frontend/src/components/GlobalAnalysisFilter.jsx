...
function customerTypeOptionLabel(item) {
  const raw = String(optionLabel(item) || '').trim();
  if (/^\d+$/.test(raw)) return `Chưa cấu hình danh mục (${raw})`;
    ...
    <div className="global-scope-bar">
      ...
      <div className="global-scope-controls">
        ...
        <div className="global-scope-actions">
          ...
          <Button className="global-scope-submit" loading={scope.sessionLoading} disabled={!draft.periodKey || scope.sessionLoading} type="primary" icon={<SearchOutlined />} onClick={apply}>{scope.sessionLoading ? 'Đang chuẩn bị' : 'Xem dữ liệu'}</Button>
          <Button
            className="global-scope-refresh"
            disabled={scope.sessionLoading}
            icon={<ReloadOutlined />}
            title="Xóa điều kiện đang chọn; dữ liệu đã tải vẫn được giữ nguyên"
            aria-label="Đặt lại bộ lọc"
            onClick={() => {
              scope.resetDraft();
              message.success('Đã xóa điều kiện lọc. Dữ liệu đang xem vẫn được giữ nguyên.');
            }}
          />
        </div>
      </div>
    </div>
    <Drawer rootClassName="global-filter-drawer" title={<div><strong>Bộ lọc phân tích nâng cao · Kỳ {draft.periodKey}</strong><small>Điều kiện được áp dụng đồng bộ cho tất cả màn hình và danh sách chi tiết</small></div>} width="min(900px, 96vw)" open={open && Boolean(draft.periodKey)} onClose={() => setOpen(false)} extra={<Space><Button icon={<ReloadOutlined />} onClick={() => { scope.resetAdvanced(); message.success('Đã xóa riêng các điều kiện nâng cao'); }}>Làm mới bộ lọc</Button><Button type="primary" icon={<SearchOutlined />} onClick={apply}>Áp dụng & xem dữ liệu</Button></Space>}>
      <div className="global-scope-advanced">
        <section className="global-filter-section is-customer"><header><TeamOutlined /><span><strong>Khách hàng & quản lý</strong><small>Nhận diện và phạm vi quan hệ</small></span></header><div className="global-filter-grid"><label className="is-wide">Tên hoặc mã khách hàng<Input allowClear placeholder="Chỉ tìm tên khách hàng hoặc mã KH lõi" value={draft.advanced.keyword} onChange={(event) => scope.updateAdvanced({ keyword: event.target.value })} /></label><label>Loại khách hàng<Select mode="multiple" allowClear maxTagCount="responsive" value={draft.advanced.customerTypes} onChange={(customerTypes) => scope.updateAdvanced({ customerTypes })} options={(options.customer_types || []).map((item) => ({ value: optionValue(item), label: customerTypeOptionLabel(item) }))} /></label><label className="is-wide">Cán bộ quản lý<Select mode="multiple" allowClear showSearch optionFilterProp="label" maxTagCount="responsive" placeholder={draft.pgdCode ? 'Cán bộ thuộc phòng ban đã chọn' : draft.branchCode ? 'Cán bộ thuộc chi nhánh đã chọn' : 'Chọn chi nhánh để thu hẹp cán bộ'} value={draft.advanced.officerCodes} onChange={(officerCodes) => scope.updateAdvanced({ officerCodes })} options={(options.officers || []).map((item) => ({ value: item.code || item.value, label: item.label || (item.name ? `${item.name} (${item.code})` : optionLabel(item)) }))} /></label><label>Trạng thái CBQL<Select allowClear value={draft.advanced.officerStatus} onChange={(officerStatus) => scope.updateAdvanced({ officerStatus: officerStatus || null })} options={[{ value: 'missing', label: 'Chưa có CBQL hợp lệ' }, { value: 'assigned', label: 'Đã có CBQL hợp lệ' }]} /></label><label>Quan hệ chi nhánh<Select allowClear value={draft.advanced.relationshipStatus} onChange={(relationshipStatus) => scope.updateAdvanced({ relationshipStatus: relationshipStatus || null })} options={[{ value: 'single', label: 'Một chi nhánh' }, { value: 'multi', label: 'Đa chi nhánh' }]} /></label><label>Chi nhánh chính<Select allowClear value={draft.advanced.primaryBranchStatus} onChange={(primaryBranchStatus) => scope.updateAdvanced({ primaryBranchStatus: primaryBranchStatus || null })} options={[{ value: 'unclear', label: 'Đa CN chưa rõ CN chính' }, { value: 'clear', label: 'Đã xác định hợp lệ' }]} /></label><label>Thời điểm xuất hiện<Select allowClear value={draft.advanced.newCustomerStatus} onChange={(newCustomerStatus) => scope.updateAdvanced({ newCustomerStatus })} options={[{ value: 'new', label: 'Khách hàng mới trong kỳ' }, { value: 'existing', label: 'Đã có từ kỳ trước' }]} /></label><label>Thông tin liên hệ<Select allowClear value={draft.advanced.contactStatus} onChange={(contactStatus) => scope.updateAdvanced({ contactStatus: contactStatus || null })} options={[{ value: 'available', label: 'Có số điện thoại' }, { value: 'missing', label: 'Thiếu số điện thoại' }]} /></label></div></section>
